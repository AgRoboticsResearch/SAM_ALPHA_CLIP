...
now_folder = 'val'  #now.strftime('%Y%m%d%H%M')
out_folder = 'F:\doctor\distill_fundation_model\output_all\output_label'  #修改成你保存分割掩码的路径
mask_folder_path = os.path.join(out_folder, now_folder)
print(mask_folder_path) 
os.makedirs(mask_folder_path, exist_ok=True)
#print('这是分割mask的文件夹mask_folder_path', mask_folder_path)

sam_checkpoint = "F:\doctor\distill_fundation_model\checkpoints\sam_vit_h_4b8939.pth"
model_type = "vit_h"
device = "cuda"
imgdir_path = 'F:\doctor\strawberry\StrawDI_Db1\StrawDI_Db1/val/img'   #修改成你自己的原始图像图像路径
...
now_folder = 'val'
label_output_path = os.path.join('F:\doctor\distill_fundation_model\output_all', now_folder)  #修改成你自己的保存txt文件的路径
...
for file_mask in files_mask:
    ...
    image_path1 = os.path.join(imgdir_path, file_mask + '.png')
    #print(image_path1)
